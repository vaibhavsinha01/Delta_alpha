...
def calculate_signals(df):
    try:
        ...
        # print(df)
        # print(df.tail(1))
        # df.to_csv('ETHUSD_Indicator.csv')
        # df.to_csv("data/ETHUSD_Indicator_main.csv")
        
        # THIS CODE COMMENTED IS NOT UNDERSTOOD YET
        ...
        for i in range(len(df)):
            ...
            if gaizy_changed:
                ...
                if current_gaizy in ['light_green','green']: # light green is bright_green,green is dark_green
                    used_gaizy_green = False
            ...
            # Only execute if no RF + IB_Box signal was triggered
                ...
                if current_gaizy in ['light_green', 'green'] and not used_gaizy_green:
                    ...
                    # Blue is neutral signal -> Triggers both boxes
                    if row['RedArrow'] == 1 or row['GreenArrow'] == 1:
                        signal = 1
            ...
            if signal != 0:
                if current_gaizy in ['light_green', 'green']:
                    used_gaizy_green = True
...
def execute_signals(df):
    try:
        ...
        if last_signal != 0:
            # print(f"der would be placed since last signal is {last_signal}")
