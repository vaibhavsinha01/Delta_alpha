...
class MartingaleManager:
    def __init__(self, base_capital, base_leverage=1):
        ...
        self.current_capital = base_capital  # Capital that grows with RM2
        ...
        self.last_gain_amount = 0  # Track last gain for RM2
    ...
    def get_trade_amount(self):
        """Calculate trade amount - always use current capital (RM2)"""
        return self.current_capital
    
    def update_trade_result(self, result, gain_amount=0):
        """Update trade result and adjust system based on RM1 and RM2"""
        ...
        if result == 'win':
            ...
            # RM2: Add Y/31 to capital for next trade
            if gain_amount > 0:
                capital_increase = gain_amount / 31
                self.current_capital += capital_increase
                self.last_gain_amount = gain_amount
                print(f"Trade won! Gain: {gain_amount}, Capital increase: {capital_increase}")
                print(f"New capital: {self.current_capital}, Resetting to level 0, leverage: {self.get_leverage()}x")
            else:
    ...
    def check_order_execution(self, binance_client, symbol, current_price, tp_price, sl_price, entry_price):
        """Check if orders were executed and determine win/loss"""
        try:
            ...
            if len(self.position_history) > 0:
                ...
                if last_position != 0 and current_position == 0:
                    ...
                    if tp_distance < sl_distance:
                        # Calculate gain amount for RM2
                        gain_amount = abs(tp_price - entry_price) * last_position
                        print(f"Position closed at TP level. Current price: {current_price}, TP: {tp_price}")
                        self.update_trade_result('win', gain_amount)
                    else:
...
# Initialize Martingale Manager with RM1 and RM2 system
...
while True:
    ...
    if entry_signal:
        ...
        # RM1 & RM2: Use current capital and set leverage
        current_capital = martingale_manager.get_trade_amount()  # Current capital (RM2)
        ...
        trade_amount = round(current_capital / entry_price, 5)
        
        print(f"RM1 System - Level: {martingale_manager.current_level}, Leverage: {current_leverage}x")
        print(f"RM2 System - Capital: {current_capital}")
    ...
    print(f"RM2 System - Current Capital: {martingale_manager.current_capital}")
