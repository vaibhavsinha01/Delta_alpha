...
class MartingaleManager:
    def __init__(self, base_capital, base_leverage=1):
        ...
        # New attributes for position management
        self.h_pos = 0  # 0: no position, 1: buy position, -1: sell position
        self.min_lot = 0.01  # Minimum lot size for Bitcoin
        
    def calculate_trade_size(self, current_price):
        """Calculate ideal trading size using the specified formula"""
        # Formula: price = ((capital / min_lot * last_price) * leverage) / 2
        leverage = self.get_leverage()
        trade_size = ((self.base_capital / self.min_lot * current_price) * leverage) / 2
        return trade_size
            ...
            try:
                ...
                if self.last_direction == 'buy':
                    ...
                    if self.last_tp_price and current_price >= self.last_tp_price:
                        ...
                        self.clear_orders()
                        self.clear_position()  # Clear position after trade closes
                        # Sleep to prevent immediate re-entry
                        print("Sleeping for 30 seconds to prevent immediate re-entry...")
                        time.sleep(30)
                        break
...
while True:
    ...
    # Check if we can take a trade (no existing position) and have a signal
    if entry_signal and martingale_manager.can_take_trade() and not martingale_manager.monitoring_active:
        ...
        # Calculate trade size using new formula
        calculated_trade_size = martingale_manager.calculate_trade_size(entry_price)
        ...
        if symbol_info:
            ...
            # Use the calculated trade size
            raw_trade_amount = calculated_trade_size / entry_price
            ...
            print(f"Calculated trade size: {calculated_trade_size}, Raw amount: {raw_trade_amount}, Adjusted amount: {trade_amount}")
        else:
            print("Could not get symbol info, using default precision")
            trade_amount = round(calculated_trade_size / entry_price, 3)  # Default to 3 decimal places
        ...
        print(f"Current position status: h_pos = {martingale_manager.h_pos}")
        
        binance_client.get_account_balance()
        
        # Place 3 separate orders
        try:
            # Set position status before placing orders
            ...
            if position_order and position_order_id:
                ...
                if sl_order_id and tp_order_id:
                    ...
                    # Cancel position order if SL/TP failed and clear position
        ...
        print(f"Signal detected ({entry_signal}) but cannot take trade - existing position: h_pos = {martingale_manager.h_pos}")
    
    elif entry_signal and martingale_manager.monitoring_active:
        print(f"Signal detected ({entry_signal}) but monitoring is active, skipping trade")
