...
class MartingaleManager:
    ...
    def check_for_fake_trade(self,side,time_candle):
        try:
            start_balance = binance_client.get_account_balance()
            count = 0
            while count<self.max_fake_count:
                try:
                    limit = 1000
                    ...
                    last_time = last_row['time'] # gives the time for the latest candle if the time_candle(when trade was placed) and the (last_time) is not the same then we can say that the order has passed

                    if last_signal == 0:
                        # binance_client.cancel_trade(symbol='BTCUSDT',order_id=self.position_order_id)
                        if side == "BUY" or side == "buy":
                            closing_side = "SELL"
                        else:
                            closing_side = "BUY"
                        binance_client.place_order(symbol=BINANCE_SYMBOL,side=closing_side,type="MARKET",quantity=self.last_quantity) # placing an order of opposite magnitude to close it 
                        self.clear_position()
                        print(f"current position is {self.h_pos}")
                        current_balance = binance_client.get_account_balance() # get the account balance and send it on the increment fake loss function
                        fake_amount = start_balance - current_balance
                        self.increment_fake_loss(fake_amount)
                        time.sleep(60) # so no trade is taken as per this candle
                        break

                    elif last_time != time_candle:
                        break

                    else:
                        print(f"The signal is still there {last_signal}")
                        time.sleep(20)
                        count += 1
                    
                    if count >= self.max_fake_count:
                        print(f"consecutive success for checking the orders for {self.max_fake_count} with time of 20seconds total time = {int(self.max_fake_count)*20}")
                        break
                except Exception as inner_e:
                    print(f"Error in fake trade check iteration: {inner_e}")
                    count += 1
                    time.sleep(5)
        except Exception as e:
