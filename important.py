...
def calculate_signals(df):
    try:
        ...
        # Configuration parameter for lookback window
        n_candles_lookback = N_CANDLE_LOOKBACK  # You can adjust this value as needed
        ...
        # Initialize RSI signal tracking variables for different priorities
        pending_rsi_buy = False
        pending_rsi_sell = False
        rsi_signal_candle = -1

        # Initialize RSI + IB_Box signal tracking variables (NEW - Signal Type 4)
        pending_rsi_buy_ib = False
        pending_rsi_sell_ib = False
        rsi_ib_signal_candle = -1
        rsi_ib_used = False
        ...
        for i in range(len(df)):
            ...
            new_rsi_buy = row['rsi_buy'] == 1
            new_rsi_sell = row['rsi_sell'] == 1
            ...
            # Update RSI + IB_Box signal tracking (NEW - Signal Type 4)
            if new_rsi_buy:
                pending_rsi_buy_ib = True
                rsi_ib_signal_candle = i
                rsi_ib_used = False
            elif new_rsi_sell:
                pending_rsi_sell_ib = True
                rsi_ib_signal_candle = i
                rsi_ib_used = False

            # Reset RF signals if they're older than n candles and not used
            if (i - rf_signal_candle) > n_candles_lookback and (last_rf_buy_signal or last_rf_sell_signal):
                last_rf_buy_signal = False
                last_rf_sell_signal = False
                rf_used = False

            # Reset Arrow signals if they're older than n candles and not used
            if (i - arrow_signal_candle) > n_candles_lookback and (last_green_arrow or last_red_arrow):
                last_green_arrow = False
                last_red_arrow = False
                arrow_used = False

            # Reset RSI + IB_Box signals if they're older than n candles and not used (NEW)
            if (i - rsi_ib_signal_candle) > n_candles_lookback and (pending_rsi_buy_ib or pending_rsi_sell_ib):
                pending_rsi_buy_ib = False
                pending_rsi_sell_ib = False
                rsi_ib_used = False
            ...
            # Condition A2: RF signal, then Arrow signal within next n candles
            elif last_rf_buy_signal and not rf_used and row['GreenArrow'] == 1 and (i - rf_signal_candle) <= n_candles_lookback:
                signal = 2  # RF + IB_Box buy signal
                rf_used = True
                last_rf_buy_signal = False
                # print(f"RF + IB_Box BUY signal triggered at candle {i} (RF at {rf_signal_candle}, Arrow at {i})")
            elif last_rf_sell_signal and not rf_used and row['RedArrow'] == 1 and (i - rf_signal_candle) <= n_candles_lookback:
                signal = -2  # RF + IB_Box sell signal
                rf_used = True
                last_rf_sell_signal = False
            ...
            # Condition B1: Arrow signal, then RF signal within next n candles
            elif last_green_arrow and not arrow_used and new_rf_buy and (i - arrow_signal_candle) <= n_candles_lookback:
                signal = 2  # RF + IB_Box buy signal
                arrow_used = True
                last_green_arrow = False
                # print(f"RF + IB_Box BUY signal triggered at candle {i} (Arrow at {arrow_signal_candle}, RF at {i})")
            elif last_red_arrow and not arrow_used and new_rf_sell and (i - arrow_signal_candle) <= n_candles_lookback:
                signal = -2  # RF + IB_Box sell signal
                arrow_used = True
                last_red_arrow = False
                # print(f"RF + IB_Box SELL signal triggered at candle {i} (Arrow at {arrow_signal_candle}, RF at {i})")

            # === PRIORITY 2: RSI + IB_Box Logic (NEW - Signal Type 4) ===
            # Only execute if no RF + IB_Box signal was triggered
            elif signal == 0:
                # Check for IB_Box confirmation after RSI signal within n candles
                if pending_rsi_buy_ib and not rsi_ib_used and row['GreenArrow'] == 1 and (i - rsi_ib_signal_candle) <= n_candles_lookback:
                    signal = 4  # RSI + IB_Box buy signal
                    pending_rsi_buy_ib = False
                    rsi_ib_used = True
                    # print(f"RSI + IB_Box BUY signal triggered at candle {i} (RSI at {rsi_ib_signal_candle}, IB_Box at {i})")
                elif pending_rsi_sell_ib and not rsi_ib_used and row['RedArrow'] == 1 and (i - rsi_ib_signal_candle) <= n_candles_lookback:
                    signal = -4  # RSI + IB_Box sell signal
                    pending_rsi_sell_ib = False
                    rsi_ib_used = True
                    # print(f"RSI + IB_Box SELL signal triggered at candle {i} (RSI at {rsi_ib_signal_candle}, IB_Box at {i})")

            # === PRIORITY 3: RSI Buy/sell + RF Logic ===
            # Only execute if no higher priority signal was triggered
            if signal == 0:
                # Track RSI signals for RF confirmation
                if new_rsi_buy:
