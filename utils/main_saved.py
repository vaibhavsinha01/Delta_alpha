...
if __name__ == "__main__":
    try:
        ...
        logger.info(f"The trading bot has started with initial capital {base_capital} and initial leverage {base_leverage} and symbol {DELTA_SYMBOL} and interval {DELTA_INTERVAL} with tp percent {DELTA_TP_PERCENT} and sl points {DELTA_SL_BUFFER_POINTS} current fake loss amount is {fake_loss_amount} and current martingale level is {martingale_manager.current_level} with fake loss threshold {DELTA_FAKE_LOSS_MAX_AMOUNT}")
        print("=" * 50)
